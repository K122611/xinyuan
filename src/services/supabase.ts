...
export function getSupabase(): SupabaseClient {
  if (!_supabase) {
    ...
    // 兜底：防止 createClient 因空 URL 直接抛异常
    const url = supabaseUrl || 'https://placeholder.supabase.co';
    const key = supabaseAnonKey || 'placeholder-key';

    // 检测非 ASCII 字符：HTTP headers 只支持 ISO-8859-1，中文会导致 fetch 崩溃
    if (/[^\x00-\x7F]/.test(key)) {
      const msg = `❌ VITE_SUPABASE_ANON_KEY 包含非法字符（中文/全角等）！
当前值: ${key.slice(0, 30)}...
请替换为纯 ASCII 的 anon-key（在 Supabase Dashboard → Project Settings → API → anon public）
当前占位符已自动替换为安全值，但注册/登录功能将不可用。`;
      console.error(msg);
      // 自动替换为安全占位符，防止后续 fetch 崩溃
      _supabase = createClient(url, 'PLACEHOLDER_ANON_KEY', {
        auth: {
