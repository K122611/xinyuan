...
class XiaozhiBridge extends EventEmitter {
  constructor(options = {}) {
    ...
    // 中继模式：注入 XiaozhiRelay 实例后，所有设备消息透明转发到官方云服务器
    this._relay = null;
  }
  ...
  setRelay(relay) {
    this._relay = relay;
    if (relay) {
      console.log(`[小智桥接] ✅ 已绑定中继实例，模式: ${relay.enabled ? '已启用' : '待启用'}`);
    } else {
      console.log('[小智桥接] 🔓 已解绑中继实例，切换回本地 AI 模式');
    }
  }
  ...
  _handleConnection(ws, req) {
    ...
    ws.on('message', (data) => {
      ...
      if (Buffer.isBuffer(data) || data instanceof ArrayBuffer) {
        ...
        if (!clientRef.info) {
          ...
          // 中继模式：通知中继有新设备连接
          if (this._relay) {
            this._relay.onDeviceConnected(clientRef.info.sessionId, ws, {
              type: 'hello',
              version: 1,
              transport: 'websocket',
            });
    ...
    ws.on('close', (code, reason) => {
      ...
      if (clientRef.info) {
        ...
        // 中继模式：通知中继设备已断开
        if (this._relay) {
          this._relay.onDeviceDisconnected(clientRef.info.sessionId);
        }
      }
    });
  ...
  _handleAudio(ws, clientInfo, buf) {
    ...
    // 中继模式：转发到官方云服务器，不本地处理
    if (this._relay && this._relay.onDeviceMessage(sessionId, buf, true)) {
      return;
    }
  ...
  _handleJSON(ws, clientRef, msg, setupTimer) {
    ...
    if (type === 'hello') {
      ...
      // 中继模式：通知中继有新设备连接
      if (this._relay) {
        this._relay.onDeviceConnected(sessionId, ws, msg);
      }
    ...
    if (type === 'listen') {
      ...
      // 中继模式：转发 listen 消息
      if (this._relay && clientRef.info && this._relay.onDeviceMessage(clientRef.info.sessionId, JSON.stringify(msg), false)) {
        return;
      }
      return;
    }

    // ── 中继模式：非 hello 的 JSON 消息转发到官方服务器 ──
    if (this._relay && clientRef.info) {
      if (this._relay.onDeviceMessage(clientRef.info.sessionId, JSON.stringify(msg), false)) {
        return;
      }
    }
