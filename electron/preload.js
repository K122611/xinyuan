...
// ========== 主进程日志转发 API ==========
// 让用户在 F12 Console 看到 [AI对话] [小智桥接] [Coze] [STT] 等主进程日志
contextBridge.exposeInMainWorld('mainLogAPI', {
  onLog: (callback) => {
    const handler = (_event, data) => callback(data);
...
contextBridge.exposeInMainWorld('relayAPI', {
  ...
  // 获取状态 { enabled, serverUrl, activeRelays, devices }
  getStatus: () => ipcRenderer.invoke('relay:status'),
  // 切换开/关
  toggle: () => ipcRenderer.invoke('relay:toggle'),
  // 监听中继事件 (started, stopped, serverDisconnected, serverError)
  onEvent: (callback) => {
    const handler = (_event, data) => callback(data);
